...
namespace VerificationCode.Code
{
    public class VerificationCodeImage
    {
        ...
        public static Task<VerificationCodeModel> CreateHanZi(int numbers = 5, int _height = 200, int _width = 200)
        {
            ...
            //string[] fonts = { "华文彩云", "Segoe Print", "Comic Sans MS", "MV Boli", "华文行楷" };
string[] fonts = { "SimSun", "STSong", "NSimSun"   };
            ...
            if(!System.IO.File.Exists(_random_file_image))
            {
                throw new Exception("image not exists:"+ _random_file_image);
            }
            FileStream fileStream = new FileStream(_random_file_image, FileMode.Open, FileAccess.Read);
